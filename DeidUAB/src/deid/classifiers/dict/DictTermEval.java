...
//		recordSet.loadFolder(new String[] { "113-05.xml" },true);
		...
		// term were matched criteria
		for(String term:hmTerm2Eval.keySet()){
			if(termsSet.contains(term)){
				
				EvalRecord<IAnnotation> evalRec=hmTerm2Eval.get(term);
				if(evalRec.FPs.size()<10){ // filter terms 
					usedTerms.add(term);

				} else {
//					Debug.error(term);
//					Debug.error(hmTerm2Eval.get(term).toString());
				}
		...
		// terms were never appeared.
		Set<String> unusedTerms=new HashSet<>(termsSet); 
		unusedTerms.removeAll(hmTerm2Eval.keySet());
		
		usedTerms.addAll(unusedTerms);
		
		FileUtil.writeFile(StringFormat.listToString(unusedTerms,"\n"), HumanNames.HUMAN_NAME_LEXICON_SELECTED, false);
		
		
		
		Debug.print(HumanNames.HUMAN_NAME_LEXICON_SELECTED);
